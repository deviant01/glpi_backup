...
if(strpos($_SERVER['PHP_SELF'],"dropdownTapes.php")){
   //$AJAX_INCLUDE = 1;
   define('GLPI_ROOT', '../../..');
...
if (!is_array($_POST['used'])) {
   ...
   if (is_array($_POST['used']) && count($_POST['used'])>0) {
      $query = "SELECT `id`
                ...
                WHERE `id` IN (".implode(',',$_POST['used']).")
                      AND `plugin_backups_tapetypes_id` = '".$_POST["plugin_backups_tapetypes_id"]."'";
   ...
   Dropdown::show('PluginBackupsTape',
                  array('name'      => $_POST['myname'],
                        'used'      => $used,
                        'entity'    => $_POST['entity'],
                        'rand'      => $_POST['rand'],
                        'condition' => "glpi_plugin_backups_tapes.plugin_backups_tapetypes_id='".$_POST["plugin_backups_tapetypes_id"]."'"));
